 ...
 * specification.
 */
public class ImmutableListMap<K, V> implements ImmutableMap<K, V> {
    /*
     * Rep invariant bindings != null bindings contains no null elements, keys
     * or values no duplicate keys
     */
    private ImmutableList<Binding> bindings;
    ...
    public ImmutableListMap () {
        this.bindings = new EmptyImmutableList<Binding>();
        checkRep();
    }

    // Internal constructor.
    private ImmutableListMap (ImmutableList<Binding> bindings) {
     ...
     */
    private Binding get(ImmutableList<Binding> bindings, Object key) {
        if (bindings.size() == 0)
            return null;
    ...
    public ImmutableMap<K, V> put(K key, V value) {
        return new ImmutableListMap<K, V>(put(bindings, key, value));
    }
     ...
     * added
     */
    private ImmutableList<Binding> put(ImmutableList<Binding> bindings, K key, V value) {
        if (bindings.size() == 0)
            return new NonEmptyImmutableList<Binding>(new Binding(key, value));
        Binding b = bindings.first();
        if (b.key.equals(key)) {
